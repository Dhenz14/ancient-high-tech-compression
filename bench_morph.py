"""
Benchmark: V6 Morphological Fallback vs V4 baseline.

Compares:
  - v4 (current best): sequential encode -> brotli q=11
  - v6 (morph fallback): encode_v6 -> brotli q=11
  - raw brotli: brotli q=11 on UTF-8 bytes

Reports pre-brotli sizes, post-brotli sizes, morph compression counts, and delta.
Any text that regresses means Phase 5 needs more investigation.
"""
import sys
sys.path.insert(0, '.')
...
from src.morphology.lemmatizer import Lemmatizer, MORPH_BASE
from src.sequential.encoder import C_LOWER, C_TITLE, C_UPPER, C_MIXED, L_NONE, _V4_RARE_TRAIL

d = Dictionary()
enc4 = SequentialEncoder(d)
dec = SequentialDecoder(d)
lemmatizer = Lemmatizer(d)
...
def count_morph_compressed(text):
    """Count how many extra-section words were morph-compressed in this text."""
    tokens = enc4._tokenize(text)
    count = 0
    bytes_saved = 0
    for original, word, caps, lead, trail in tokens:
        ...
        if not go_extra:
            continue
        if (rank is None and not is_rare_trail and lead == L_NONE
                and caps in (C_LOWER, C_TITLE)):
            base, morph_flag = lemmatizer.lemmatize(word)
            if morph_flag != MORPH_BASE and d.has_word(base):
                base_rank = d.word_to_rank(base)
                if base_rank is not None:
                    from src.sequential.morph_encoder import _varint_len
                    morph_size = 1 + _varint_len(base_rank) + 1
                    raw_size = 1 + len(original.encode('utf-8'))
                    if morph_size < raw_size:
                        count += 1
                        bytes_saved += raw_size - morph_size
    return count, bytes_saved


print("=" * 85)
print("PHASE 5: V6 MORPHOLOGICAL FALLBACK vs V4 BASELINE")
...
for name, text in texts.items():
    blob6 = encode_v6(text, d)
    result = dec.decode(blob6)
    ok = (result == text)
    if not ok:
        all_ok = False
        ...
        n_morph, pre_saved = count_morph_compressed(text)
        print("  %-12s PASS  (%d morph-compressed, %d bytes pre-brotli saved)"
              % (name, n_morph, pre_saved))

if not all_ok:
    print("\nROUND-TRIP FAILURES — Phase 5 is BROKEN.")
    sys.exit(1)
...
for name, text in texts.items():
    ...
    blob6 = encode_v6(text, d)
    v6_raw = len(blob6)
    v6_br = len(brotli.compress(blob6, quality=11))
...
print("-" * 85)
total_delta = total_v6 - total_v4
print("%-12s %6s | %8s %8s | %8d %8d | %s" % (
    "TOTAL", "", "", "",
    total_v4, total_v6,
...
print("=" * 85)
if regressions == 0:
    print("PHASE 5: PASSES ✓ — %d improvements, %d bytes saved total" % (improvements, abs(total_delta)))
    if total_delta < 0:
        print("Next step: lock as v3.0, proceed to Phase 6 (SA re-training)")
    else:
        print("Net neutral — small improvement possible with more aggressive morphology")
else:
    print("PHASE 5: %d regressions, %d improvements, net %+d bytes" % (regressions, improvements, total_delta))
    if total_delta < 0:
        print("Net improvement despite regressions — investigate regressions before locking")
    else:
        print("Net negative — DITCH Phase 5 or investigate root cause")
print("=" * 85)
